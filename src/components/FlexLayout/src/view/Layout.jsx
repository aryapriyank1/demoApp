...
export class Layout extends React.Component {
    ...
    if (this.state.showEdges) {
      ...
      edges.push(
        <div
          ...
          className={className + ' ' + this.getClassName(CLASSES.FLEXLAYOUT__EDGE_RECT_TOP)}></div>
      );
      edges.push(
        <div
          key="West"
          ...
          className={className + ' ' + this.getClassName(CLASSES.FLEXLAYOUT__EDGE_RECT_LEFT)}></div>
      );
      edges.push(
        <div
          key="South"
          ...
          className={
            className + ' ' + this.getClassName(CLASSES.FLEXLAYOUT__EDGE_RECT_BOTTOM)
          }></div>
      );
      edges.push(
        <div
          ...
          className={
            className + ' ' + this.getClassName(CLASSES.FLEXLAYOUT__EDGE_RECT_RIGHT)
          }></div>
      );
    }
        ...
        for (const child of drawChildren) {
          if (child instanceof SplitterNode) {
            ...
            splitterComponents.push(
              <Splitter key={child.getId()} layout={this} node={child} path={path} />
            );
  ...
  renderChildren(path, node, tabSetComponents, tabComponents, floatingWindows, splitterComponents) {
    ...
    for (const child of drawChildren) {
      if (child instanceof SplitterNode) {
        ...
        splitterComponents.push(
          <Splitter key={child.getId()} layout={this} path={newPath} node={child} />
        );
        ...
        // is row
        const newPath =
          path + (child.getOrientation() === Orientation.HORZ ? '/r' : '/c') + rowCount++;
        this.renderChildren(
          newPath,
          child,
          tabSetComponents,
  ...
  handleCustomTabDrag(dropInfo, pos, event) {
    ...
    this.customDrop = undefined;
    const dragging =
      this.newTabJson || (this.dragNode instanceof TabNode ? this.dragNode : undefined);
    if (
      dragging &&
      (dropInfo.node instanceof TabSetNode || dropInfo.node instanceof BorderNode) &&
      dropInfo.index === -1
    ) {
